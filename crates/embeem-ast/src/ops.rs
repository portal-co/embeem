...
/// Target types for virtual function calls.
///
/// These represent the "object" in virtual function call syntax like `WRITE(GPIO(pin), value)`.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub enum Target {
    /// GPIO pin
    Gpio,
...
impl Target {
    ...
    pub fn from_str(s: &str) -> Option<Target> {
        match s {
            "GPIO" => Some(Target::Gpio),
            "ADC" => Some(Target::Adc),
            "DAC" => Some(Target::Dac),
            "PWM" => Some(Target::Pwm),
            "TIMER" => Some(Target::Timer),
            "UART" => Some(Target::Uart),
            "SPI" => Some(Target::Spi),
            "I2C" => Some(Target::I2c),
            "CAN" => Some(Target::Can),
            "USB" => Some(Target::Usb),
            "DMA" => Some(Target::Dma),
            "WDT" => Some(Target::Wdt),
            "EEPROM" => Some(Target::Eeprom),
            "FLASH" => Some(Target::Flash),
            "RTC" => Some(Target::Rtc),
            _ => None,
        }
    }
...
/// Virtual operation types for virtual function call syntax.
///
/// These represent the "method" in virtual function call syntax like `WRITE(GPIO(pin), value)`.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
pub enum VirtualOp {
    /// Read from target
    Read,
...
impl VirtualOp {
    /// Parse a virtual operation from its string name.
    pub fn from_str(s: &str) -> Option<VirtualOp> {
        match s {
            "READ" => Some(VirtualOp::Read),
            "WRITE" => Some(VirtualOp::Write),
            "INIT" => Some(VirtualOp::Init),
            "START" => Some(VirtualOp::Start),
            "STOP" => Some(VirtualOp::Stop),
            "RESET" => Some(VirtualOp::Reset),
            "TOGGLE" => Some(VirtualOp::Toggle),
            "SET_MODE" => Some(VirtualOp::SetMode),
            "SET_FREQUENCY" => Some(VirtualOp::SetFrequency),
            "SET_DUTY_CYCLE" => Some(VirtualOp::SetDutyCycle),
            "SET_CLOCK" => Some(VirtualOp::SetClock),
            "SET_BAUD_RATE" => Some(VirtualOp::SetBaudRate),
            "TRANSFER" => Some(VirtualOp::Transfer),
            "AVAILABLE" => Some(VirtualOp::Available),
            "FLUSH" => Some(VirtualOp::Flush),
            "ENABLE" => Some(VirtualOp::Enable),
            "DISABLE" => Some(VirtualOp::Disable),
            "SET_TIMEOUT" => Some(VirtualOp::SetTimeout),
            "SET_PERIOD" => Some(VirtualOp::SetPeriod),
            "SET_COMPARE" => Some(VirtualOp::SetCompare),
            "SET_RESOLUTION" => Some(VirtualOp::SetResolution),
            "SET_REFERENCE" => Some(VirtualOp::SetReference),
            "SET_PULSE_WIDTH" => Some(VirtualOp::SetPulseWidth),
            _ => None,
        }
    }

    /// Resolve a virtual operation with a target into a concrete OpKind.
    pub fn resolve(self, target: Target) -> Option<OpKind> {
        match (self, target) {
            // GPIO operations
            (VirtualOp::Read, Target::Gpio) => Some(OpKind::GpioRead),
            (VirtualOp::Write, Target::Gpio) => Some(OpKind::GpioWrite),
            (VirtualOp::Toggle, Target::Gpio) => Some(OpKind::GpioToggle),
            (VirtualOp::SetMode, Target::Gpio) => Some(OpKind::GpioSetMode),

            // ADC operations
            (VirtualOp::Read, Target::Adc) => Some(OpKind::AdcRead),
            (VirtualOp::SetResolution, Target::Adc) => Some(OpKind::AdcSetResolution),
            (VirtualOp::SetReference, Target::Adc) => Some(OpKind::AdcSetReference),

            // DAC operations
            (VirtualOp::Write, Target::Dac) => Some(OpKind::DacWrite),

            // PWM operations
            (VirtualOp::Start, Target::Pwm) => Some(OpKind::PwmStart),
            (VirtualOp::Stop, Target::Pwm) => Some(OpKind::PwmStop),
            (VirtualOp::SetDutyCycle, Target::Pwm) => Some(OpKind::PwmSetDutyCycle),
            (VirtualOp::SetFrequency, Target::Pwm) => Some(OpKind::PwmSetFrequency),
            (VirtualOp::SetPulseWidth, Target::Pwm) => Some(OpKind::PwmSetPulseWidth),

            // Timer operations
            (VirtualOp::Start, Target::Timer) => Some(OpKind::TimerStart),
            (VirtualOp::Stop, Target::Timer) => Some(OpKind::TimerStop),
            (VirtualOp::Reset, Target::Timer) => Some(OpKind::TimerReset),
            (VirtualOp::Read, Target::Timer) => Some(OpKind::TimerRead),
            (VirtualOp::SetPeriod, Target::Timer) => Some(OpKind::TimerSetPeriod),
            (VirtualOp::SetCompare, Target::Timer) => Some(OpKind::TimerSetCompare),

            // UART operations
            (VirtualOp::Init, Target::Uart) => Some(OpKind::UartInit),
            (VirtualOp::Write, Target::Uart) => Some(OpKind::UartWriteByte),
            (VirtualOp::Read, Target::Uart) => Some(OpKind::UartReadByte),
            (VirtualOp::Available, Target::Uart) => Some(OpKind::UartAvailable),
            (VirtualOp::Flush, Target::Uart) => Some(OpKind::UartFlush),
            (VirtualOp::SetBaudRate, Target::Uart) => Some(OpKind::UartSetBaudRate),

            // SPI operations
            (VirtualOp::Init, Target::Spi) => Some(OpKind::SpiInit),
            (VirtualOp::Transfer, Target::Spi) => Some(OpKind::SpiTransfer),
            (VirtualOp::SetMode, Target::Spi) => Some(OpKind::SpiSetMode),
            (VirtualOp::SetClock, Target::Spi) => Some(OpKind::SpiSetClock),

            // I2C operations
            (VirtualOp::Init, Target::I2c) => Some(OpKind::I2cInit),
            (VirtualOp::Start, Target::I2c) => Some(OpKind::I2cStart),
            (VirtualOp::Stop, Target::I2c) => Some(OpKind::I2cStop),
            (VirtualOp::Write, Target::I2c) => Some(OpKind::I2cWrite),
            (VirtualOp::Read, Target::I2c) => Some(OpKind::I2cRead),
            (VirtualOp::SetClock, Target::I2c) => Some(OpKind::I2cSetClock),

            // CAN operations
            (VirtualOp::Init, Target::Can) => Some(OpKind::CanInit),
            (VirtualOp::Write, Target::Can) => Some(OpKind::CanSend),
            (VirtualOp::Read, Target::Can) => Some(OpKind::CanReceive),

            // USB operations
            (VirtualOp::Init, Target::Usb) => Some(OpKind::UsbInit),
            (VirtualOp::Write, Target::Usb) => Some(OpKind::UsbWrite),
            (VirtualOp::Read, Target::Usb) => Some(OpKind::UsbRead),
            (VirtualOp::Available, Target::Usb) => Some(OpKind::UsbAvailable),

            // DMA operations
            (VirtualOp::Init, Target::Dma) => Some(OpKind::DmaInit),
            (VirtualOp::Start, Target::Dma) => Some(OpKind::DmaStart),
            (VirtualOp::Stop, Target::Dma) => Some(OpKind::DmaStop),

            // Watchdog operations
            (VirtualOp::Enable, Target::Wdt) => Some(OpKind::WdtEnable),
            (VirtualOp::Disable, Target::Wdt) => Some(OpKind::WdtDisable),
            (VirtualOp::Reset, Target::Wdt) => Some(OpKind::WdtReset),
            (VirtualOp::SetTimeout, Target::Wdt) => Some(OpKind::WdtSetTimeout),

            // EEPROM operations
            (VirtualOp::Read, Target::Eeprom) => Some(OpKind::EepromRead),
            (VirtualOp::Write, Target::Eeprom) => Some(OpKind::EepromWrite),

            // Flash operations
            (VirtualOp::Read, Target::Flash) => Some(OpKind::FlashRead),
            (VirtualOp::Write, Target::Flash) => Some(OpKind::FlashWrite),

            // RTC operations
            (VirtualOp::Init, Target::Rtc) => Some(OpKind::RtcInit),
            (VirtualOp::Read, Target::Rtc) => Some(OpKind::RtcGetTime),
            (VirtualOp::Write, Target::Rtc) => Some(OpKind::RtcSetTime),
